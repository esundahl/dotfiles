...
#  * feature, bug and refactor branches
#  * remote feature, bug and refactor branch names
# 
# 
# Installation
# ------------
...
_git_flow ()
{
	local subcommands="init feature refactor bug help version"
...
__git_flow_refactor ()
{
	...
	case "$subcommand" in
	finish)
		__gitcomp "$(__git_flow_list_branches 'refactor')"
		return
		;;
	publish)
		__gitcomp "$(comm -23 <(__git_flow_list_branches 'refactor') <(__git_flow_list_remote_branches 'refactor'))"
		return
		;;
	track)
		__gitcomp "$(comm -23 <(__git_flow_list_remote_branches 'refactor') <(__git_flow_list_branches 'refactor'))"
		return
		;;
	*)
		COMPREPLY=()
...
__git_flow_bug ()
{
	...
	case "$subcommand" in
	finish)
		__gitcomp "$(__git_flow_list_branches 'bug')"
		return
		;;
	*)
		COMPREPLY=()
